...
int main()
{
    try
    {
        ...
        DBResultsRAII result {connection.getQueryResults("SELECT * FROM CUSTOMER_TABLE")};
        DBResultsRAII result_two {std::move(result)};
        
        if (result_two)
        {
            std::vector<Customer> customers = readCustomers(result_two);
