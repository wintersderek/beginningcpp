...
void sort(std::vector<T> &data, Compare comp, size_t start, size_t end)
{
    ...
    if (current)
        sort(data, comp, start, current - 1); // Sort left subset if exists
    sort(data, comp, current + 1, end);       // Sort right subset if exists
}
